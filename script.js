...
clearBtn.onclick = () => {
    ...
    gererRechercheAdresses(valeurRecherche) {
        ...
        document.querySelectorAll('.bras-details').forEach(details => {
            ...
            const cardsVisibles = details.querySelectorAll('.address-card:not([style*="display: none"])');
    ...
    verifierAvertissementDonnees() {
        ...
        if (clearStorageBtn) {
            clearStorageBtn.style.display = this.gestionnaireDonnees.aDesDonnees() ? 'inline-block' : 'none';
        }

        // Le bouton addAddressBtn doit faire 100% en largeur s'il n'y a aucune adresse
        const addAddressBtn = document.getElementById('addAddressBtn');
        if (addAddressBtn) {
            addAddressBtn.style.width = this.gestionnaireDonnees.aDesDonnees() ? '' : '100%';
        }

        // Le bouton exportExcelBtn doit être caché s'il n'y a aucune adresse
        const exportExcelBtn = document.getElementById('exportExcelBtn');
        if (exportExcelBtn) {
            exportExcelBtn.style.display = this.gestionnaireDonnees.aDesDonnees() ? 'inline-block' : 'none';
        }

        // Le bouton importExcelBtn doit prendre toute la largeur s'il n'y a pas d'adresse
        const importExcelBtn = document.getElementById('importExcelBtn');
        if (importExcelBtn) {
            importExcelBtn.style.width = this.gestionnaireDonnees.aDesDonnees() ? '' : '100%';
        }
        ...
        if (addressSearchWrapper) {
            addressSearchWrapper.style.display = this.gestionnaireDonnees.aDesDonnees() ? 'flex' : 'none';
        }
    ...
    rafraichirInterface() {
        ...
        const aUnFichier = this.gestionnaireDonnees.nomFichier && this.gestionnaireDonnees.nomFichier.length > 0;
        if (titreFichier) {
        ...
        if (cardsContent) {
            ...
            if (brasTries.length === 0) {
                cardsContent.innerHTML = '<p class="no-data-msg">Aucune adresse à afficher</p>';
            }
        }
    ...
    modifierAdresse(index) {
        ...
        document.getElementById('addressPopupTitle').textContent = 'Modifier l\'adresse';
