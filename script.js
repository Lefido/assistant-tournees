...
class GestionnaireDonnees {
    ...
    return new Promise((resolve, reject) => {
      ...
      lecteur.onload = (evt) => {
        try {
          ...
          const donneesBrutes = XLSX.utils.sheet_to_json(
            classeur.Sheets[feuille],
          );

          this.donneesExcel = donneesBrutes.map((ligne) => ({
            BRAS: String(ligne.BRAS || "")
              .trim()
              .toLowerCase(),
            ...
            Adresse: String(ligne.Adresse || "")
              ...
              .toLowerCase(),
            Numero: String(
              ligne["Numéro de tournée"] || ligne["Numéro"] || "",
            ).trim(),
  ...
  rechercherAdresses(termeRecherche, typeRecherche = null) {
    ...
    return this.donneesExcel.filter(
      (r) =>
  ...
  obtenirBrasUniques() {
    return [...new Set(this.donneesExcel.map((l) => l.BRAS))]
      .filter((b) => b)
      .sort();
  }
  ...
  obtenirVillesPourBras(bras) {
    return [
      ...new Set(
        this.donneesExcel.filter((r) => r.BRAS === bras).map((r) => r.Ville),
      ),
    ]
      .filter((v) => v)
      .sort();
...
class GestionnaireReconnaissanceVocale {
  constructor(gestionnaireDonnees) {
    ...
    this.instance = this.reconnaissanceVocale
      ? new this.reconnaissanceVocale()
      : null;
  ...
  confirmerRechercheVocale() {
    const resultats = this.gestionnaireDonnees.rechercherAdresses(
      this.dernierReconnu,
    );
...
class GestionnaireCamera {
  ...
  async _capturerEtTraiter() {
    ...
    try {
      ...
      const resultat = await Tesseract.recognize(this.canvasElement, "fra", {
        logger: (m) => {
          console.log(m);
          if (this.statusElement && m.status === "recognizing text") {
            this._mettreAJourStatut(
              `Analyse... ${Math.round(m.progress * 100)}%`,
            );
          }
        },
        rectangle: rectangle,
  ...
  _analyserAdresseDepuisTexte(texte) {
    ...
    const regexRue =
      /\b(rue|boulevard|bd|avenue|av|place|pl|chemin|impasse|allee|route|rt|voie|square|sq|cours|imp|passage|pass|quai|pont|carrefour|car|résidence|res|lotissement|lot|zone|zn|parc|prk)\b/i;

    for (const ligne of lignes) {
      const correspondance = ligne.match(regexCodePostal);
      if (correspondance) {
        const apresCode = ligne
          .substring(correspondance.index + correspondance[0].length)
          .replace(/[^a-zA-Z\s-]/g, "")
          .trim();
      ...
      if (regexRue.test(ligne)) {
        ...
        const mots = nettoye
          ...
          .filter(
            (m) =>
              m.length > 2 &&
              !/\b(le|la|les|du|de|des|et|à|a|sur|chez|pour|avec)\b/i.test(m),
          );
        rue = mots.join(" ");
        if (rue) break;
      }
...
class GestionnaireInterface {
  ...
  initialiserRecherche() {
    ...
    champRecherche.addEventListener("focus", () => {
      ...
      boutonEffacer.style.top =
        80 + champRecherche.offsetHeight / 2 - 12 + "px";
  ...
  gererRechercheTempsReel(valeurRecherche) {
    const valeurNormalisee = this.gestionnaireDonnees.normaliserTexte(
      valeurRecherche.trim(),
    );
    const divResultats = document.getElementById("liveSearchResults");
    document.getElementById("clearSearchBtn").style.display = valeurNormalisee
      ? "flex"
      : "none";

    if (
      valeurNormalisee.length < 2 ||
      !this.gestionnaireDonnees.brasSelectionne
    ) {
    ...
    let resultatsFiltres = this.gestionnaireDonnees.rechercherAdresses(
      valeurNormalisee,
      "1",
    );
  ...
  initialiserRechercheAdresses() {
    const champRecherche = document.getElementById("addressSearchInput");
    const boutonEffacer = document.getElementById("clearAddressSearchBtn");
  ...
  gererRechercheAdresses(valeurRecherche) {
    const boutonEffacer = document.getElementById("clearAddressSearchBtn");
    if (boutonEffacer) {
      boutonEffacer.style.display = valeurRecherche ? "flex" : "none";
    }
    ...
    cards.forEach((card) => {
      ...
      if (texteCarte.includes(valeurNormalisee)) {
        ...
        while (parent && !parent.classList.contains("bras-details")) {
    ...
    document.querySelectorAll(".bras-details").forEach((details) => {
      ...
      const cardsVisibles = details.querySelectorAll(
        '.address-card:not([style*="display: none"])',
      );

      if (cardsVisibles.length > 0) {
        details.open = true;
      } else {
        // Garder ouvert si c'était un résultat de recherche
        if (brasTrouve.has(details)) {
          details.open = true;
        }
      }
    });
  ...
  rafraichirInterface() {
    ...
    const aUnFichier =
      this.gestionnaireDonnees.nomFichier &&
      this.gestionnaireDonnees.nomFichier.length > 0;
    if (titreFichier) {
      titreFichier.style.display = aUnFichier ? "block" : "none";
    }

    // Le conteneur des adresses est toujours visible (titre et bouton)
    // Seul le contenu (les cartes) dépend de la présence de données
  ...
  selectionnerBras(bras, bouton) {
    ...
    document
      .querySelectorAll("#brasBtnContainer .city-btn")
  ...
  restaurerBrasSelectionne() {
    ...
    const boutonsBras = document.querySelectorAll(
      "#brasBtnContainer .city-btn",
    );
  ...
  modifierAdresse(index) {
    ...
    document.getElementById("addressBras").value = adresse.BRAS || "";
    document.getElementById("addressVille").value = adresse.Ville || "";
    document.getElementById("addressRue").value = adresse.Adresse || "";
    document.getElementById("addressNumero").value = adresse.Numero || "";
    document.getElementById("addressType").value = adresse.TypeRecherche || "1";
    ...
    saveAddressBtn.onclick = () => {
      ...
      this._derniereValeurRecherche = addressSearchInput
        ? addressSearchInput.value
        : "";
  ...
  afficherColorPicker(bras, ville, couleurActuelle) {
    ...
    GestionnaireInterface.COULEURS.forEach((couleur) => {
      ...
      colorOption.className =
        "color-option" + (couleur === couleurActuelle ? " selected" : "");
  ...
  selectionnerCouleur(couleur) {
    ...
    const colorElement = document.querySelector(
      `.pdf-city-color[data-ville="${ville}"]`,
    );
    if (colorElement) {
    ...
    villesTriees.forEach((ville, index) => {
      ...
      adressesFiltreesVille.forEach((adresse) => {
        ...
        const couleurLigne = estSpecial
          ...
          : alterner
            ? `background-color:${couleurVille}20;`
            : "";
        alterner = !alterner;
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  document
    ...
    .addEventListener("change", gererImportExcel);

  // Gestion des modales et panneaux
  document.getElementById("confirmBtn").onclick = () =>
    gestionnaireReconnaissance.confirmerRechercheVocale();
  document.getElementById("retryBtn").onclick = () =>
    gestionnaireReconnaissance.reessayerReconnaissance();
  document.getElementById("cancelBtn").onclick = () =>
    gestionnaireReconnaissance.annulerReconnaissance();
